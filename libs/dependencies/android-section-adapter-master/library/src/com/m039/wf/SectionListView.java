...
public class SectionListView extends ListView {
	...
	void initScrollListener() {
		super.setOnScrollListener(new OnScrollListener() {
			public void onScroll(AbsListView view, int firstVisibleItem,
				...
				SectionListView.this.onScroll(view, firstVisibleItem,
						visibleItemCount, totalItemCount);
			}
			...
			public void onScrollStateChanged(AbsListView view, int scrollState) {
				...
				SectionListView.this.onScrollStateChanged(view, scrollState);
			}
		});
	}
	...
	public void setAdapter(ListAdapter adapter) {
		...
		mPinnedView = a.getView(a.getSectionsAtPosition(0)[1], mPinnedView,
				mOverlay);
	...
	void onScroll(final AbsListView listView, final int firstVisibleItem,
		...
		new Runnable() {
			...
			public void run() {
				...
				if (prevPosition != null && prevPosition != spositions[1]) {
					mPinnedView = a.getView(spositions[1], mPinnedView,
							mOverlay);
			...
			void log() {
				...
				sb.append("  fvi: " + firstVisibleItem + " vic: "
						...
						if (v != null) {
							...
							if (t < b) {
								...
								if (mOnPushedUpListener != null)
									mOnPushedUpListener.onPushedUp(mPinnedView,
											1f - (b - t) / (float) b);
			...
			void showNextSections(int curPosition) {
				...
				while (curPosition != -1 && isNextSectionVisible(curPosition)) {
			...
			boolean isNextSectionVisible(int curPosition) {
				return ((firstVisibleItem < curPosition) && (curPosition < firstVisibleItem
						+ visibleItemCount));
			}
	...
	SectionAdapter getSectionAdapter() {
		try {
			...
			throw new RuntimeException(
					"You should use SectionListView with SectionAdapter!");
		}
	}
