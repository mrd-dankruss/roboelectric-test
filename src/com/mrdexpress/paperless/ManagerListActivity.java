...
public class ManagerListActivity extends ListActivity implements LoaderCallbacks<Cursor>,
		OnQueryTextListener
{
	...
	private static final int URL_LOADER = 0;// Identifies a particular Loader being used in this
											// component

	static final String[] FROM =
	{ DbHandler.C_MANAGER_NAME };
	static final int[] TO =
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		setTitle(R.string.title_actionbar_select_manager);
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id)
			{
				if (holder.list.getItemAtPosition(position) != null)
				{
					...
					Intent intent = new Intent(getApplicationContext(),
							ManagerAuthIncompleteScanActivity.class);

					DbHandler.getInstance(getApplicationContext());
					intent.putExtra(VariableManager.EXTRA_MANAGER_NAME,
	...
	public Loader<Cursor> onCreateLoader(int id, Bundle args)
	{
		...
		String rawQuery = "SELECT * FROM " + DbHandler.TABLE_MANAGERS + " WHERE "
				+ DbHandler.C_MANAGER_NAME + " LIKE " + " '%" + filter + "%'" + " ORDER BY "
				+ DbHandler.C_MANAGER_NAME + " ASC";
