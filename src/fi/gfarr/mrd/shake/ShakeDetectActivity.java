...
/**
 * 
 * A shake detection library. To use, have your Activity create an instance of
 * the class and set a listener using addListener(); Make sure you call
 * onResume() and onPause() on from the activity
 * 
 * @author James
...
public class ShakeDetectActivity implements SensorEventListener {
	...
	public void onSensorChanged(SensorEvent event) {
		if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
			...
			if (lastShake != 0
					&& (curTime - lastShake) < IGNORE_EVENTS_AFTER_SHAKE)
				return;
			...
			if (last_x != 0 && last_y != 0 && last_z != 0
				...
				DataPoint dp = new DataPoint(last_x - x, last_y - y,
						last_z - z, curTime);
	...
	public void checkForShake() {
		...
		while (dataPoints.size() > 0
				&& dataPoints.get(0).atTimeMilliseconds < cutOffTime)
			dataPoints.remove(0);
